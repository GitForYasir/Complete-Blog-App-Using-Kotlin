...
class BlogAdapter(private val items: MutableList<BlogItemModel>): RecyclerView.Adapter<BlogAdapter.BlogViewHolder>() {
    ...
    private val databaseReference: DatabaseReference = FirebaseDatabase.getInstance("https://blog-app-6337a-default-rtdb.asia-southeast1.firebasedatabase.app").reference
    ...
    inner class BlogViewHolder(private val binding: BlogItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(blogItemModel: BlogItemModel) {
            ...
            binding.likeCount.text = blogItemModel.likeCounter.toString()
            ...
            // checked if the current user has liked the post and update the like button image
            ...
            val currentUserLiked = currentUser?.uid?.let { uid->
                postLikeReference.child(uid).addListenerForSingleValueEvent(object : ValueEventListener{
                    override fun onDataChange(snapshot: DataSnapshot) {
                        if (snapshot.exists()){
                            binding.likeBtn.setImageResource(R.drawable.heart_red)
                        }else{
            ...
            postSaveReference.addListenerForSingleValueEvent(object :ValueEventListener{
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()){
                        ...
                        binding.postSaveBtn.setImageResource(R.drawable.save_artivle_fill_red)
                    }else{
                        // if blog not yet saved
                        binding.postSaveBtn.setImageResource(R.drawable.unsave_article)
                    }
                }
                    ...
                    if (snapshot.exists()){
                        userReference.child("likes").child(postId).removeValue()
                            .addOnSuccessListener {
                                ...
                                val newLikeCount = blogItemModel.likeCounter - 1
                                blogItemModel.likeCounter = newLikeCount
                                databaseReference.child("blogs").child(postId).child("likeCounter").setValue(newLikeCount)
        ...
        userReference.child("saveBlogPosts").child(postId).addListenerForSingleValueEvent(object :ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()){
                    ...
                    // the blog is not saved yet, so saved it
